...
function save_event_ajax(brief, details) {
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4) {
            ...
            if (this.status == 200) {
                ...
                msg.style.visibility = 'visible';
                msg.innerHTML = "Sorry, an error occurred.";
            }
        }
    }
    ...
    $('#fscreen').scrollTo(0, 0);
    msg.style.visibility = 'visible';
    msg.innerHTML = '<div class="pending"></div>Processing ...';
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4) {
            if (this.status == 200) {
                ...
                if (response.success === true) {
                    pagination_refresh_ajax('curr');
                    edit_event_func(false, true, $forms('edit_event', 'event_id').value);
                }
            } else {
                msg.style.visibility = 'visible';
                msg.innerHTML = "Sorry, an error occurred.";
            ...
            if (this.status == 200) {
                ...
                if (evnt["uploads"].length > 0) {
                    ...
                    for (i = 0; i < evnt["uploads"].length; i++) {
                        ...
                        edit_text += "<div class='file_pare'><input type='text' placeholder='Add a caption' name='edit_cap" + (i + 1) + "' value='" + evnt["uploads"][i]["caption"] + "' class='input2' autocomplete='off'>" + tag + "<div class='action_pare'><span class='del'>Delete this.</span></div></div>";
                    }
        ...
        if (this.readyState == 4) {
            if (this.status == 200) {
                ...
                if (evnt["uploads"].length > 0) {
                    ...
                    for (i = 0; i < evnt["uploads"].length; i++) {
                        if (evnt["uploads"][i]["type"] == "vid") {
                            tag += "<div class='preview_cont'><span class='next' onclick='next_slide(event, this);'>&#10095;</span><span class='prev' onclick='prev_slide(event, this)'>&#10094;</span><div class='file'><video muted><source src='/lifelog/stream/?id=" + evnt["uploads"][i]["id"] + "'>Sorry, your browser can't play this video.</source></video></div><div class='caption_cont'>" + evnt["uploads"][i]["caption"] + "</div></div>";
                        } else {
                ...
                $('#fscreen_action').innerHTML = "<span class='icon icon3 icon_20px edit' onclick=\"edit_event_func(this, true, 'e" + event_id + "')\" title='Edit'></span><span class='icon icon4 icon_20px del' onclick=\"trash_event(this, 'e" + event_id + "')\" title='Move to Trash'></span>"
...
function pagination_refresh_ajax(action) {
    ...
    xmlhttp.open("GET", "/lifelog/ajax/pagination/?nav=" + action, true);
    xmlhttp.send();
}
...
function element_listeners() {
    ...
    // $forms('search').onsubmit = function (event) {
    //     event.preventDefault();
    //     search_ajax();
    // }
    ...
    $forms('edit_event', 'brief').oninput = function () {
        ...
        $("#e_msg").outerHTML = '<span id="e_msg"></span>';
    }
    ...
    $forms('new_event', 'brief').oninput = function () {
        ...
        $("#msg").outerHTML = '<span id="msg"></span>';
    }
...
function event_listeners() {
    ...
    //     }
    // })
    $all('.event .ent_cont .checkbox').forEach(function (val, ind, arr) {
    ...
    $all('.event .ent_cont .action .edit').forEach(function (val, ind, arr) {
        val.onclick = function (event) {
            event.stopImmediatePropagation();
            edit_event_func(this, false, this.parentNode.parentNode.parentNode.id);
        };
    })
    $all('.event .ent_cont .action .del').forEach(function (val, ind, arr) {
        val.onclick = function (event) {
            event.stopImmediatePropagation();
            trash_event(this, this.parentNode.parentNode.parentNode.id);
        };
    })
}
